...
- [x] 2. ストレージサービスの実装
  ...
  - [x]* 2.3 ストレージサービスのユニットテストを作成する
    - localStorage未対応時の動作テスト
    - データ未存在時の初期状態テスト
    - データ破損時のリカバリテスト
    - _Requirements: 5.4, 5.5_

- [x] 3. チェックポイント - コア基盤の確認
...
- [x] 4. 共通フレームワークUIコンポーネントの実装
  ...
  - [x]* 4.5 進捗表示フォーマットのプロパティテストを作成する
...
- [x] 5. 問題画面とフィードバックの実装
  ...
  - [x]* 5.4 問題画面とフィードバックのユニットテストを作成する
...
- [x] 7. 回転図形問題プラグインのロジック実装
  - [x] 7.1 回転図形問題の型定義を作成する
    ...
    - `Grid` 型（`[boolean, boolean, boolean, boolean]`）、`RotationDirection` 型（`'right1' | 'left1' | 'right2' | 'left2'`）、`RotationQuestionData`、`RotationChoiceData` を定義する
    - _Requirements: 6.1_
