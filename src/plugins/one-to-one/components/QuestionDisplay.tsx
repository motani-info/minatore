      ...
      <Box>
        <Flex wrap="wrap" justify="center" gap={1}>
          {Array.from({ length: data.topCount }).map((_, i) => (
            <Text key={i} fontSize="28px" lineHeight="1">{data.topEmoji}</Text>
          ))}
        </Flex>
      </Box>

      <Text fontSize="sm" color="gray.400" fontWeight="600">↕</Text>

      {/* 下のアイテム */}
      <Box>
        <Flex wrap="wrap" justify="center" gap={1}>
          {Array.from({ length: data.bottomCount }).map((_, i) => (
            <Text key={i} fontSize="28px" lineHeight="1">{data.bottomEmoji}</Text>
          ))}
        </Flex>
      </Box>
    </VStack>
